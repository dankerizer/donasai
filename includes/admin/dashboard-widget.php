...
function wpd_dashboard_widget_render() {
	...
	?>
	<div class="wpd-widget-stats" style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap:10px; margin-bottom:15px; text-align:center;">
		<div style="background:#f0fdf4; padding:10px; border-radius:5px;">
            <p style="color:#166534; font-size:12px; margin:0;">Total Donasi</p>
			<strong style="font-size:18px; color:#15803d;">Rp <?php echo number_format( $total_collected, 0, ',', '.' ); ?></strong>
		</div>
		<div style="background:#eff6ff; padding:10px; border-radius:5px;">
            <p style="color:#1e40af; font-size:12px; margin:0;">Donatur</p>
			<strong style="font-size:18px; color:#1d4ed8;"><?php echo number_format( $total_donors ); ?></strong>
		</div>
        <div style="background:#fff7ed; padding:10px; border-radius:5px;">
            <p style="color:#9a3412; font-size:12px; margin:0;">Campaigns</p>
			<strong style="font-size:18px; color:#c2410c;"><?php echo number_format( $active_campaigns ); ?></strong>
		</div>
	</div>

    <div class="wpd-widget-recent">
        <h4 style="margin:0 0 10px; font-size:13px; color:#666; border-bottom:1px solid #eee; padding-bottom:5px;">Donasi Terbaru</h4>
        <?php if ( $recent ) : ?>
            <ul style="list-style:none; margin:0; padding:0;">
                <?php foreach ( $recent as $row ) : ?>
                    <li style="margin-bottom:8px; display:flex; justify-content:space-between; font-size:13px;">
                        ...
                        <span style="font-weight:bold; color:#059669;">+ Rp <?php echo number_format( $row->amount, 0, ',', '.' ); ?></span>
                    </li>
                <?php endforeach; ?>
            </ul>
        <?php else : ?>
    ...
    <div style="margin-top:15px; padding-top:10px; border-top:1px solid #eee; text-align:right;">
        <a href="<?php echo admin_url( 'admin.php?page=wpd-donations' ); ?>" class="button button-primary button-small">Lihat Semua</a>
    </div>
	<?php
}
