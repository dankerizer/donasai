 ...
 */
function donasai_add_dashboard_widgets()
{
    wp_add_dashboard_widget(
        'donasai_dashboard_widget',
        'Donasai Overview',
        'donasai_dashboard_widget_render'
    );
}
add_action('wp_dashboard_setup', 'donasai_add_dashboard_widgets');

/**
 * Render Widget Content
 */
function donasai_dashboard_widget_render()
{
    // Check Cache
    $cache_key = 'donasai_dashboard_stats_unified';
    $stats = wp_cache_get($cache_key, 'donasai_dashboard');

    if (false === $stats) {
        $stats = array();
        $overview = donasai_get_stats_overview();
        
        $stats['total_collected'] = $overview['total_collected'];
        $stats['total_donors'] = $overview['total_donors'];
        $stats['active_campaigns'] = $overview['active_campaigns'];
        $stats['recent'] = donasai_get_recent_donations(5);
    ...
    ?>
    <div class="donasai-widget-stats"
    ...
    <div class="donasai-widget-recent">
    ...
    <div style="margin-top:15px; padding-top:10px; border-top:1px solid #eee; text-align:right;">
        <a href="<?php echo esc_url(admin_url('admin.php?page=donasai-donations')); ?>"
