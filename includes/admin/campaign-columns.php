...
function wpd_campaign_custom_column( $column, $post_id ) {
    switch ( $column ) {
        ...
        case 'stats':
            ...
            echo '<div style="margin-bottom:4px;"><strong>Target:</strong> Rp ' . number_format( (float)$target, 0, ',', '.' ) . '</div>';
            echo '<div style="margin-bottom:4px;"><strong>Collected:</strong> <span style="color:'.$color.'">Rp ' . number_format( (float)$collected, 0, ',', '.' ) . '</span></div>';
            ...
            echo '<div style="background:'.$color.';height:100%;width:'.min(100, $percent).'%;"></div>';
            echo '</div>';
            break;
        ...
        case 'dates':
            ...
            echo '<div><strong>Start:</strong> ' . $published . '</div>';
            if ( $deadline ) {
                $days_left = ceil( ( strtotime( $deadline ) - time() ) / 86400 );
                if( $days_left < 0 ) {
                    echo '<div style="color:#dc2626;"><strong>End:</strong> ' . date_i18n( 'd M Y', strtotime( $deadline ) ) . ' (Expired)</div>';
                } else {
                    echo '<div><strong>End:</strong> ' . date_i18n( 'd M Y', strtotime( $deadline ) ) . '</div>';
                }
            } else {
        ...
        case 'wpd_actions':
            ...
            // Edit
            echo '<a href="' . get_edit_post_link( $post_id ) . '" style="display:block; padding:8px 12px; text-decoration:none; color:#1d2327; border-bottom:1px solid #f0f0f1; transition:bg 0.2s;" onmouseover="this.style.background=\'#f6f7f7\'" onmouseout="this.style.background=\'#fff\'">Edit Campaign</a>';
...
function wpd_handle_recalc_stats() {
    ...
    if ( ! wp_verify_nonce( $_GET['_wpnonce'], 'wpd_recalc_stats_' . $post_id ) ) {
    ...
    // Redirect back
    wp_redirect( admin_url( 'edit.php?post_type=wpd_campaign&updated=true' ) );
    exit;
}
