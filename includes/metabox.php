...
function wpd_campaign_options_callback( $post ) {
	...
	<div class="wpd-meta-box">
        ...
        if ( ! empty( $pro_accounts ) ) {
            ...
            ?>
            <hr>
            <h4><?php _e( 'Bank Accounts (Manual Transfer)', 'donasai' ); ?></h4>
            <p class="description"><?php _e( 'Select which bank accounts to display for this campaign. Leave empty to use global defaults.', 'donasai' ); ?></p>
            <div style="background:#fff; border:1px solid #ddd; padding:10px; max-height:150px; overflow-y:auto;">
                <?php foreach ( $pro_accounts as $acc ) : ?>
                    <label style="display:block; margin-bottom:5px;">
                        ...
                        <strong><?php echo esc_html( $acc['bank_name'] ); ?></strong> - <?php echo esc_html( $acc['account_number'] ); ?> 
                        <?php if ( ! empty( $acc['is_default'] ) ) echo '<span style="font-size:10px; background:#eee; padding:2px 4px; border-radius:3px; margin-left:5px;">Default</span>'; ?>
                    </label>
                <?php endforeach; ?>
            </div>
            <?php
